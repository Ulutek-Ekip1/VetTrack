        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return Scaffold(
              ...
              body: Center(
                child: Text('Ziyaret bilgileri alınamadı: ${snapshot.error}'),
              ),
            );
          }
            ...
            body: RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  ...data.history.map((visit) => ListTile(
                        leading: Icon(
                          visit.isOngoing ? Icons.pending : Icons.check_circle,
                          color: visit.isOngoing ? Colors.orange : Colors.teal,
                        ),
                        title: Text(
                          visit.isOngoing
                              ? 'Devam eden muayene'
                        ...
                        subtitle: Text(
                          'Başlangıç: ${visit.startedAt.toLocal()}',
                        ),
                      )),
                  const SizedBox(height: 16),
                  const Text(
                        ...
                        children: treatments
                            .map((t) => ListTile(
                                  leading: const Icon(Icons.medication,
                                      color: Colors.teal),
                                  title: Text(t.title),
                                  subtitle: Text(t.description ?? t.type),
                                ))
                            .toList(),
                      );
                    },
